...
class aTanh(torch.nn.Tanh):
    ...
    def forward(self, inputs, grad: torch.Tensor = None):
        ...
        transformations combined with this transformation.
        """
        inputs = (inputs/torch.sqrt((inputs).pow(2).sum(1))) #<-- fix this!
        g = 2*torch.log(torch.cosh(inputs))
        return 0.5*(torch.log(1+inputs) - torch.log(1-inputs)), (g.view(grad.shape) + grad) if grad is not None else g
